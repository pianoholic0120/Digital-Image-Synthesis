...
OptiXAggregate::BVH OptiXAggregate::buildBVHForGaussians(
    const std::vector<ShapeSceneEntity> &shapes, OptixDeviceContext optixContext,
    const OptixProgramGroup &intersectPG, const OptixProgramGroup &shadowPG,
    const OptixProgramGroup &randomHitPG,
    const std::map<std::string, FloatTexture> &floatTextures,
    ...
    ThreadLocal<Allocator> &threadAllocators,
    ThreadLocal<cudaStream_t> &threadCUDAStreams) {
    std::vector<size_t> cloudIndexToShapeIndex;
    for (size_t i = 0; i < shapes.size(); ++i) {
        if (shapes[i].name == "gaussiancloud")
            cloudIndexToShapeIndex.push_back(i);
    }

    size_t nClouds = cloudIndexToShapeIndex.size();
    if (nClouds == 0)
        return {};
    ...
    std::vector<std::vector<Gaussian3D>> hostClouds(nClouds);
    for (size_t cloudIndex = 0; cloudIndex < nClouds; ++cloudIndex) {
        const auto &shape = shapes[cloudIndexToShapeIndex[cloudIndex]];
        std::string filename =
            ResolveFilename(shape.parameters.GetOneString("filename", ""));
        if (filename.empty())
            ErrorExit(&shape.loc, "gaussiancloud: \"filename\" parameter not specified.");
        Float sigmaCutoff = shape.parameters.GetOneFloat("sigma_cutoff", 2.828f);
        hostClouds[cloudIndex] = Load3DGSPly(filename, sigmaCutoff);
        if (hostClouds[cloudIndex].empty())
            ErrorExit(&shape.loc, "gaussiancloud: no Gaussians loaded from %s.", filename);
        totalGaussians += int(hostClouds[cloudIndex].size());
    }

    std::vector<int> cloudAABBStartIndex(nClouds);
    int aabbIndex = 0;
    for (size_t cloudIndex = 0; cloudIndex < nClouds; ++cloudIndex) {
        cloudAABBStartIndex[cloudIndex] = aabbIndex;
        aabbIndex += int(hostClouds[cloudIndex].size());
    }

    std::vector<OptixBuildInput> optixBuildInputs(nClouds);
    std::vector<OptixAabb> aabbs(totalGaussians);
    OptixAabb *deviceAABBs;
    CUDA_CHECK(cudaMalloc(&deviceAABBs, sizeof(OptixAabb) * totalGaussians));
    std::vector<CUdeviceptr> aabbDevicePtrs(nClouds);
    std::vector<uint32_t> flags(nClouds);

    for (size_t cloudIndex = 0; cloudIndex < nClouds; ++cloudIndex) {
        const auto &shape = shapes[cloudIndexToShapeIndex[cloudIndex]];
        const std::vector<Gaussian3D> &hostGaussians = hostClouds[cloudIndex];

        Float sigmaCutoff = shape.parameters.GetOneFloat("sigma_cutoff", 2.828f);
        int shDegree = shape.parameters.GetOneInt("sh_degree", 3);
        bool useCenterDepth = shape.parameters.GetOneBool("use_center_depth", true);

        Gaussian3D *d_gaussians = UploadGaussiansToDevice(pstd::MakeConstSpan(hostGaussians));

        int localAABBIndex = cloudAABBStartIndex[cloudIndex];
        Bounds3f cloudBounds;
        for (const auto &g : hostGaussians) {
            OptixAabb aabb = {float(g.aabb.pMin.x), float(g.aabb.pMin.y), float(g.aabb.pMin.z),
                              float(g.aabb.pMax.x), float(g.aabb.pMax.y),
                              float(g.aabb.pMax.z)};
            aabbs[localAABBIndex++] = aabb;
            cloudBounds = Union(cloudBounds, g.aabb);
        }

        OptixBuildInput &input = optixBuildInputs[cloudIndex];
        input.type = OPTIX_BUILD_INPUT_TYPE_CUSTOM_PRIMITIVES;
        input.customPrimitiveArray.numSbtRecords = 1;
        input.customPrimitiveArray.numPrimitives = hostGaussians.size();
        aabbDevicePtrs[cloudIndex] = CUdeviceptr(&deviceAABBs[cloudAABBStartIndex[cloudIndex]]);
        input.customPrimitiveArray.aabbBuffers = &aabbDevicePtrs[cloudIndex];
        input.customPrimitiveArray.flags = &flags[cloudIndex];
        flags[cloudIndex] = getOptixGeometryFlags(false, {});
        ...
        HitgroupRecord hgRecord;
        OPTIX_CHECK(optixSbtRecordPackHeader(intersectPG, &hgRecord));
        hgRecord.gaussianRec.d_gaussians = d_gaussians;
        hgRecord.gaussianRec.sigmaCutoff = sigmaCutoff;
        hgRecord.gaussianRec.shDegree = shDegree;
        hgRecord.gaussianRec.useCenterDepth = useCenterDepth;
        hgRecord.gaussianRec.reverseOrientation = shape.reverseOrientation;
        hgRecord.gaussianRec.material = material;
        hgRecord.gaussianRec.mediumInterface = getMediumInterface(shape, media, alloc);
        bvh.intersectHGRecords[cloudIndex] = hgRecord;

        OPTIX_CHECK(optixSbtRecordPackHeader(randomHitPG, &hgRecord));
        bvh.randomHitHGRecords[cloudIndex] = hgRecord;

        OPTIX_CHECK(optixSbtRecordPackHeader(shadowPG, &hgRecord));
        bvh.shadowHGRecords[cloudIndex] = hgRecord;

        bvh.bounds = Union(bvh.bounds, cloudBounds);
    }
...
OptiXAggregate::OptiXAggregate(
    ...
    UploadGaussianSobolOffsets(4096);
    ...
    OptixProgramGroup hitPGGaussian = createIntersectionPG(
        "__closesthit__gaussian", nullptr, "__intersection__gaussian");
    ...
    OptixProgramGroup anyhitPGShadowGaussian = createIntersectionPG(
        nullptr, "__anyhit__shadowGaussian", "__intersection__gaussian");
    ...
    OptixProgramGroup hitPGRandomHitGaussian = createIntersectionPG(
        "__closesthit__randomHitGaussian", nullptr, "__intersection__gaussian");
    ...
    for (const auto &shape : scene.shapes)
        if (shape.name != "sphere" && shape.name != "cylinder" && shape.name != "disk" &&
            ...
            shape.name != "curve" && shape.name != "gaussiancloud")
    ...
    AsyncJob<GAS> *gaussianJob = RunAsync([&]() {
        BVH gaussianBVH = buildBVHForGaussians(
            scene.shapes, optixContext, hitPGGaussian, anyhitPGShadowGaussian,
            hitPGRandomHitGaussian, textures.floatTextures, namedMaterials, materials,
            media, shapeIndexToAreaLights, threadAllocators, threadCUDAStreams);
        int gaussianSBTOffset = addHGRecords(gaussianBVH);
        return GAS{gaussianBVH, gaussianSBTOffset};
    });
    ...
    struct Instance {
        OptixTraversableHandle handles[4] = {};
        int sbtOffsets[4] = {-1, -1, -1, -1};
        Bounds3f bounds;
    ...
    ParallelFor(0, scene.instanceDefinitions.size(), [&](int64_t i) {
        ...
        BVH gaussianBVH = buildBVHForGaussians(
            def.second->shapes, optixContext, hitPGGaussian, anyhitPGShadowGaussian,
            hitPGRandomHitGaussian, textures.floatTextures, namedMaterials, materials,
            media, {}, threadAllocators, threadCUDAStreams);
        if (gaussianBVH.traversableHandle) {
            inst.handles[3] = gaussianBVH.traversableHandle;
            inst.sbtOffsets[3] = addHGRecords(gaussianBVH);
            inst.bounds = Union(inst.bounds, gaussianBVH.bounds);
        }
    ...
    for (AsyncJob<GAS> *job : {triJob, blpJob, quadricJob, gaussianJob}) {
...
void OptiXAggregate::IntersectClosest(int maxRays, const RayQueue *rayQueue,
    ...
    if (rootTraversable) {
        ...
        params.gaussianFrameNumber = gaussianFrameNumber;
