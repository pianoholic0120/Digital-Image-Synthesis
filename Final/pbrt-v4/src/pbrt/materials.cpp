...
std::string DielectricMaterial::ToString() const {
    return StringPrintf(
        ...
        displacement.Get(), normalMap.Get() ? normalMap.Get()->ToString() : std::string("(nullptr)"),
...
std::string ThinDielectricMaterial::ToString() const {
    return StringPrintf(
        "[ ThinDielectricMaterial displacement: %s normalMap: %s eta: %s ]", displacement.Get(),
        normalMap.Get() ? normalMap.Get()->ToString() : std::string("(nullptr)"), eta);
}
...
// GaussianMaterial Method Definitions
std::string GaussianMaterial::ToString() const {
    return StringPrintf("[ GaussianMaterial shDegree: %d cloud: %s ]", shDegree,
                        cloud ? "(bound)" : "(nullptr)");
}

GaussianMaterial *GaussianMaterial::Create(const TextureParameterDictionary &parameters,
...
std::string DiffuseMaterial::ToString() const {
    std::string normalMapString = normalMap.Get() ? normalMap.Get()->ToString() : "(nullptr)";
    return StringPrintf(
        "[ DiffuseMaterial displacement: %s normapMap: %s reflectance: %s ]",
        displacement.Get(), normalMapString, reflectance);
}
...
std::string SubsurfaceMaterial::ToString() const {
    return StringPrintf("[ SubsurfaceMaterial displacement: %s normalMap: %s scale: %f "
                        ...
                        displacement.Get(),
                        normalMap.Get() ? normalMap.Get()->ToString() : std::string("(nullptr)"),
                        scale, sigma_a, sigma_s, reflectance, mfp,
...
std::string DiffuseTransmissionMaterial::ToString() const {
    return StringPrintf("[ DiffuseTransmissionMaterial displacement: %s reflectance: %s "
                        ...
                        displacement.Get(), reflectance, transmittance, scale);
}
...
Material Material::Create(const std::string &name,
    ...
    if (name.empty() || name == "none") {
        ...
        material = GaussianMaterial::Create(parameters, normalMap, loc, alloc);
    else if (name == "mix") {
