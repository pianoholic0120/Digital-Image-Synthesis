...
// Tracks whether a material actually has a normal map; avoids returning
// uninitialized pointer values when no normal map was specified.
struct StoredNormalMap {
    bool has = false;
    Image *map = nullptr;

    StoredNormalMap() = default;
    PBRT_CPU_GPU StoredNormalMap(Image *normalMap)
        : has(normalMap != nullptr), map(normalMap) {}

    PBRT_CPU_GPU const Image *Get() const { return has ? map : nullptr; }
};

struct StoredFloatTexture {
    bool has = false;
    FloatTexture value;

    StoredFloatTexture() = default;
    PBRT_CPU_GPU StoredFloatTexture(FloatTexture texture)
        : has(bool(texture)), value(texture) {}

    PBRT_CPU_GPU FloatTexture Get() const { return has ? value : FloatTexture(); }
};
...
class DielectricMaterial {
  public:
    ...
    PBRT_CPU_GPU
    FloatTexture GetDisplacement() const { return displacement.Get(); }
    PBRT_CPU_GPU
    const Image *GetNormalMap() const { return normalMap.Get(); }
...
// GaussianMaterial Definition
class GaussianCloud;

class GaussianMaterial {
  public:
    ...
    static GaussianMaterial *Create(const TextureParameterDictionary &parameters,
                                    Image *normalMap, const FileLoc *loc, Allocator alloc);

    template <typename TextureEvaluator>
    PBRT_CPU_GPU void GetBSSRDF(TextureEvaluator, MaterialEvalContext,
                                SampledWavelengths &) const {}

    PBRT_CPU_GPU static constexpr bool HasSubsurfaceScattering() { return false; }

    template <typename TextureEvaluator>
    PBRT_CPU_GPU bool CanEvaluateTextures(TextureEvaluator) const { return true; }
