...
std::pair<double, double> InitializeByRejection(
    const std::vector<double>& image, int w, int h, std::mt19937& rng) {
    std::uniform_real_distribution<double> uni01(0.0, 1.0);
    std::uniform_int_distribution<int> uniX(0, w - 1);
    std::uniform_int_distribution<int> uniY(0, h - 1);

    // Since luminance is normalized to [0,1], f_max <= 1.
    for (int attempt = 0; attempt < 100000; ++attempt) {
        const int x = uniX(rng);
        const int y = uniY(rng);
        const double f = image[y * w + x];
        if (uni01(rng) <= f) {
            return {static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5};
        }
    }

    // Fallback to uniform if rejection sampling fails on near-black inputs.
    return {static_cast<double>(uniX(rng)) + 0.5, static_cast<double>(uniY(rng)) + 0.5};
}
...
std::vector<unsigned char> MetropolisImageCopy(
    ...
    // PDF solution #2 for startup bias: initialize using another sampling method.
    const auto initial = InitializeByRejection(image, w, h, rng);
    double cx = initial.first;
    double cy = initial.second;
