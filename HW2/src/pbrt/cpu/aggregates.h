...
class UniformGridAggregate {
  public:
    ...
    UniformGridAggregate(std::vector<Primitive> p, int nVoxelsHint = -1,
                         int maxVoxelsPerAxis = 128);
  ...
  private:
    int PosToVoxel(Point3f p, int axis) const;
    Float VoxelToPos(int v, int axis) const;
...
// TwoLevelUniformGridAggregate Definition
class TwoLevelUniformGridAggregate {
  public:
    // TwoLevelUniformGridAggregate Public Methods
    TwoLevelUniformGridAggregate(std::vector<Primitive> p, int nVoxelsHint = -1,
                                 int maxVoxelsPerAxis = 128, int subgridNVoxelsHint = -1,
                                 int subgridMaxVoxelsPerAxis = 24,
                                 int minPrimsForSubgrid = 8);

    static TwoLevelUniformGridAggregate *Create(std::vector<Primitive> prims,
                                                const ParameterDictionary &parameters);
  ...
  private:
    struct CellData {
        ...
        std::unique_ptr<UniformGridAggregate> subgrid;
    };
