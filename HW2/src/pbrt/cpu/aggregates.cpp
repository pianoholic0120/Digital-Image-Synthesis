...
// UniformGridAggregate Method Definitions
// Grid construction and 3D DDA traversal follow pbrt-v2 GridAccel (grid.cpp).
STAT_PIXEL_COUNTER("UniformGrid/Voxels visited", uniformGridVoxelsVisited);

int UniformGridAggregate::PosToVoxel(Point3f p, int axis) const {
    Float w = voxelWidth[axis];
    if (w <= 0)
        return 0;
    int v = int(std::floor((p[axis] - bounds.pMin[axis]) / w));
    return Clamp(v, 0, nVoxels[axis] - 1);
}

Float UniformGridAggregate::VoxelToPos(int v, int axis) const {
    return bounds.pMin[axis] + v * voxelWidth[axis];
}

UniformGridAggregate::UniformGridAggregate(std::vector<Primitive> p, int nVoxelsHint,
                                             int maxVoxelsPerAxis)
    : primitives(std::move(p)) {
    CHECK(!primitives.empty());
    CHECK_GT(maxVoxelsPerAxis, 0);
    bounds = Bounds3f();
    ...
    int maxExtentAxis = 0;
    for (int i = 1; i < 3; ++i)
        if (delta[i] > delta[maxExtentAxis])
            maxExtentAxis = i;
    Float maxWidth = delta[maxExtentAxis];
    if (maxWidth <= 0)
        maxWidth = 1.f;

    // Resolution from pbrt-v2 GridAccel: roughly cubic cells vs primitive count.
    Float cubeRoot = 3.f * std::pow(float(primitives.size()), 1.f / 3.f);
    Float voxelsPerUnitDist = cubeRoot / maxWidth;

    for (int axis = 0; axis < 3; ++axis) {
        int nv = int(std::round(delta[axis] * voxelsPerUnitDist));
        int cap = maxVoxelsPerAxis;
        if (nVoxelsHint > 0)
            cap = std::min(cap, nVoxelsHint);
        nv = Clamp(nv, 1, cap);
        if (delta[axis] <= 0)
            nv = 1;
        nVoxels[axis] = nv;
    ...
    for (int axis = 0; axis < 3; ++axis)
        voxelWidth[axis] =
            delta[axis] > 0 ? delta[axis] / Float(nVoxels[axis]) : Float(1.f);
        ...
        for (int z = vmin[2]; z <= vmax[2]; ++z)
            for (int y = vmin[1]; y <= vmax[1]; ++y)
                for (int x = vmin[0]; x <= vmax[0]; ++x)
                    voxels[x + nVoxels[0] * (y + nVoxels[1] * z)].push_back(int(i));
    }
}

pstd::optional<ShapeIntersection> UniformGridAggregate::Intersect(const Ray &ray,
                                                                  Float tMax) const {
    Float tMin, tFarBounds;
    if (!bounds.IntersectP(ray.o, ray.d, tMax, &tMin, &tFarBounds))
        return {};
    tMin = std::max<Float>(0.f, tMin);
    tMax = std::min(tMax, tFarBounds);

    Float rayT = tMin;
    Point3f gridIntersect = ray(rayT);

    Float nextCrossingT[3], deltaT[3];
    int step[3], pos[3], out[3];

    for (int axis = 0; axis < 3; ++axis) {
        Float dir = ray.d[axis];
        if (std::abs(dir) < 1e-8f) {
            pos[axis] = PosToVoxel(gridIntersect, axis);
            nextCrossingT[axis] = Infinity;
            deltaT[axis] = Infinity;
            step[axis] = 0;
            out[axis] = -1;
            continue;
        }
        ...
        if (dir > 0) {
            Float nextPlane = VoxelToPos(pos[axis] + 1, axis);
            nextCrossingT[axis] = rayT + (nextPlane - gridIntersect[axis]) / dir;
            deltaT[axis] = voxelWidth[axis] / dir;
            step[axis] = 1;
            out[axis] = nVoxels[axis];
        } else {
            Float nextPlane = VoxelToPos(pos[axis], axis);
            nextCrossingT[axis] = rayT + (nextPlane - gridIntersect[axis]) / dir;
            deltaT[axis] = -voxelWidth[axis] / dir;
            step[axis] = -1;
            out[axis] = -1;
    ...
    for (;;) {
        ++voxelsVisited;
        int o = pos[0] + nVoxels[0] * (pos[1] + nVoxels[1] * pos[2]);
        if (o >= 0 && o < (int)voxels.size()) {
            for (int primIdx : voxels[o]) {
                pstd::optional<ShapeIntersection> primSi = primitives[primIdx].Intersect(ray, tMax);
                if (primSi) {
                    si = primSi;
                    tMax = si->tHit;
                }
    ...
    uniformGridVoxelsVisited += voxelsVisited;
    return si;
}
...
UniformGridAggregate *UniformGridAggregate::Create(std::vector<Primitive> prims,
                                                   const ParameterDictionary &parameters) {
    int nVoxelsHint = parameters.GetOneInt("nvoxels", -1);
    int maxPerAxis = parameters.GetOneInt("maxvoxelsperaxis", 128);
    maxPerAxis = std::max(1, maxPerAxis);
    return new UniformGridAggregate(std::move(prims), nVoxelsHint, maxPerAxis);
}

// TwoLevelUniformGridAggregate Method Definitions
STAT_PIXEL_COUNTER("TwoLevelUniformGrid/Top voxels visited", twoLevelTopVoxelsVisited);

int TwoLevelUniformGridAggregate::PosToVoxel(Point3f p, int axis) const {
    Float w = voxelWidth[axis];
    if (w <= 0)
        return 0;
    int v = int(std::floor((p[axis] - bounds.pMin[axis]) / w));
...
Float TwoLevelUniformGridAggregate::VoxelToPos(int v, int axis) const {
    return bounds.pMin[axis] + v * voxelWidth[axis];
}

TwoLevelUniformGridAggregate::TwoLevelUniformGridAggregate(
    std::vector<Primitive> p, int nVoxelsHint, int maxVoxelsPerAxis, int subgridNVoxelsHint,
    int subgridMaxVoxelsPerAxis, int minPrimsForSubgrid)
    : primitives(std::move(p)) {
    CHECK(!primitives.empty());
    CHECK_GT(maxVoxelsPerAxis, 0);
    CHECK_GT(subgridMaxVoxelsPerAxis, 0);
    minPrimsForSubgrid = std::max(2, minPrimsForSubgrid);
    ...
    for (int axis = 0; axis < 3; ++axis)
        voxelWidth[axis] = delta[axis] > 0 ? delta[axis] / Float(nVoxels[axis]) : Float(1.f);
    ...
    for (size_t i = 0; i < primitives.size(); ++i) {
        ...
        for (int z = vmin[2]; z <= vmax[2]; ++z)
            for (int y = vmin[1]; y <= vmax[1]; ++y)
                for (int x = vmin[0]; x <= vmax[0]; ++x)
                    cells[x + nVoxels[0] * (y + nVoxels[1] * z)].primIndices.push_back(int(i));
    }

    for (CellData &cell : cells) {
        if ((int)cell.primIndices.size() < minPrimsForSubgrid)
            continue;

        std::vector<Primitive> localPrims;
        localPrims.reserve(cell.primIndices.size());
        for (int idx : cell.primIndices)
            localPrims.push_back(primitives[idx]);
        cell.subgrid = std::make_unique<UniformGridAggregate>(
            std::move(localPrims), subgridNVoxelsHint, subgridMaxVoxelsPerAxis);
    }
}

pstd::optional<ShapeIntersection> TwoLevelUniformGridAggregate::Intersect(const Ray &ray,
                                                                          Float tMax) const {
    Float tMin, tFarBounds;
    if (!bounds.IntersectP(ray.o, ray.d, tMax, &tMin, &tFarBounds))
        return {};
    ...
    for (;;) {
        ++voxelsVisited;
        int o = pos[0] + nVoxels[0] * (pos[1] + nVoxels[1] * pos[2]);
        if (o >= 0 && o < (int)cells.size()) {
            const CellData &cell = cells[o];
            if (cell.subgrid) {
                pstd::optional<ShapeIntersection> cellSi = cell.subgrid->Intersect(ray, tMax);
                if (cellSi) {
                    si = cellSi;
                    tMax = si->tHit;
                }
            } else {
                for (int primIdx : cell.primIndices) {
                    pstd::optional<ShapeIntersection> primSi = primitives[primIdx].Intersect(ray, tMax);
                    if (primSi) {
                        si = primSi;
                        tMax = si->tHit;
    ...
    twoLevelTopVoxelsVisited += voxelsVisited;
    return si;
}

bool TwoLevelUniformGridAggregate::IntersectP(const Ray &ray, Float tMax) const {
    Float tMin, tFarBounds;
    if (!bounds.IntersectP(ray.o, ray.d, tMax, &tMin, &tFarBounds))
        return false;
    tMin = std::max<Float>(0.f, tMin);
    ...
    for (;;) {
        ...
        if (o >= 0 && o < (int)cells.size()) {
            const CellData &cell = cells[o];
            if (cell.subgrid) {
                if (cell.subgrid->IntersectP(ray, tMax))
                    return true;
            } else {
                for (int primIdx : cell.primIndices) {
                    if (primitives[primIdx].IntersectP(ray, tMax))
...
TwoLevelUniformGridAggregate *TwoLevelUniformGridAggregate::Create(
    std::vector<Primitive> prims, const ParameterDictionary &parameters) {
    int nVoxelsHint = parameters.GetOneInt("nvoxels", -1);
    int maxPerAxis = parameters.GetOneInt("maxvoxelsperaxis", 128);
    int subgridNVoxelsHint = parameters.GetOneInt("subgrid_nvoxels", -1);
    int subgridMaxPerAxis = parameters.GetOneInt("subgrid_maxvoxelsperaxis", 24);
    int minPrimsForSubgrid = parameters.GetOneInt("subgrid_minprims", 8);
    maxPerAxis = std::max(1, maxPerAxis);
    subgridMaxPerAxis = std::max(1, subgridMaxPerAxis);
    return new TwoLevelUniformGridAggregate(std::move(prims), nVoxelsHint, maxPerAxis,
                                            subgridNVoxelsHint, subgridMaxPerAxis,
                                            minPrimsForSubgrid);
}
...
Primitive CreateAccelerator(const std::string &name, std::vector<Primitive> prims,
    ...
    else if (name == "twoleveluniformgrid")
        accel = TwoLevelUniformGridAggregate::Create(std::move(prims), parameters);
    else
